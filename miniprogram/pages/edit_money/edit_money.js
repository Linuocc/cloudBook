...
Page({
  ...
  onLoad: function (options) {
    ...
    try{
      db.collection('expenses').doc(options.id).get().then(res=>{
        let {description,length,width,price,otherMoney,totalPrices} = res.data
        this.setData({
          description,
          length,
          width,
  ...
  save(){
    ...
    if(description!==''&&(totalPrices!==0||otherMoney!==0)){
      ...
      db.collection('expenses').doc(this.data.id).update({
        data:{
          description,
          length,
          width,
